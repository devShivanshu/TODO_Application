...
class UserRepoImpl(@Autowired private  val jdbcTemplate: JdbcTemplate) : UserRepository {
    ...
    companion object {
        val SQL_CREATE = "INSERT INTO TRVS_USER(USER_ID,FIRST_NAME,LAST_NAME,EMAIL,PASSWORD) values(NEXTVAL('TRVS_USER_SEQ'),?,?,?,?)";
        val SQL_COUNT_BY_EMAIL = "SELECT COUNT(*) FROM TRVS_USER WHERE EMAIL = ?";
        val SQLFIND_BY_ID = "SELECT USER_ID, FIRST_NAME, LAST_NAME, EMAIL, PASSWORD " + "FROM TRVS_USER WHERE USER_ID =?"
